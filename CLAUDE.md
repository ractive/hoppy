...
Recorded live responses never overwrite `fixtures/` — fixture values are test contracts (decision 2026-07-10). To check the live API for drift, run the `/api-drift-sweep` skill: it records the live suite into a scratch dir (`HOPPY_RECORD_DIR=$(pwd)/fixtures-recorded cargo test --workspace --features live-api -- --test-threads=1`, single-threaded so tests don't race on filenames) and generates a read-only shape/leak report via `fixture-refresh --shape-report`. Fixture updates happen only inside an iteration that changes client types, fixtures, and tests together.
