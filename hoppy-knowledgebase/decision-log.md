...
| `--dry-run` intercepts at the client `send()`/`execute()` boundary (iter-82) | No shared HTTP core exists (10 independent execution paths: 9 service clients + `TusUploader`), so the block goes right after `RequestBuilder::build()` and before `http.execute()` in each one, gated on the existing `recording::debug::is_mutating(method)` predicate. A blocked request returns a typed `bunny_net_api::dry_run::DryRunSkipped { method, url, body }` error rather than a fake response; `main.rs` detects it by walking the `anyhow::Error` chain (survives `.context()` wrapping) and exits 0. GET/HEAD always execute, so composite commands (resolving a storage zone password, a stream library key, a container app's linked pull zones) stay truthful — only the *first* mutating request in a chain is blocked and previewed, not any follow-ups. `--dry-run` implies `--yes` (folded in `main.rs`) since the mutation is blocked regardless — no need to touch the three separate prompt implementations. One documented exception: `stream video resolutions cleanup` already had a server-side `?dryRun=true` preview via its own `StreamCleanupResolutions::dry_run` field; the global flag now drives that field directly, and `StreamClient::cleanup_video_resolutions` bypasses the client-level block via a private `send_bypassing_dry_run` so that request still reaches the API. |
...
| 2026-07-10 | `container app` stays PATCH-only — no `app replace` (PUT) command | The dead-code client method `update_application` (PUT `/apps/{appId}`, full replace) is **not** exposed as a CLI command. `container app update` (PATCH) plus the now-wired `--volume` array covers the practical need; the only thing PUT adds is clearing fields by omission (full-replace semantics), which is a footgun for a CLI (an omitted flag silently wiping templates/volumes). Keeping one mutation verb avoids a confusing PATCH-vs-PUT choice. The client method is retained (it costs nothing and documents the endpoint) but stays unreachable. Revisit only if a concrete "clear all X in one call" need appears (iter-76) |
| 2026-07-10 | Schema-less container endpoints emit raw JSON, not typed rows | `app summary` (`GET /apps/{id}/summary`), `node ips` (`GET /nodes/plain`), and `registry image-config` (`POST /registries/image-config`) are documented in the spec with **no response schema**. Rather than guess a typed model, the client methods return `serde_json::Value` and the CLI prints it pretty-printed verbatim (no `--format table` row). Live-verify the shapes during dogfooding before adding typed models. Follows the same "don't invent a schema" caution flagged in the api-coverage research §4.5 (iter-76) |
| 2026-07-10 | Probes wired via `--probes-json <file>`, not per-field flags | Container health probes are a deep nested schema (startup/readiness/liveness × httpGet/tcpSocket/grpc + five thresholds each). Full flag mapping would be ~40 flags; instead `template add/update` take a `--probes-json` file that deserializes straight into `ContainerProbes`. Entrypoint (5 fields) and pull-policy/digest are shallow enough to warrant real flags. Matches the escape-hatch recommendation in api-coverage research §4 (iter-76) |
| 2026-07-10 | Recordings never overwrite fixtures — `fixture-refresh --apply` removed | Fixtures are **test contracts** whose values the wiremock tests pin; recorded live responses are **observations**. The 2026-07-10 sweep applied 38 recorded files and had to revert 36 (value-coupled asserts, lifecycle-state capture, date-keyed chart churn). `fixture-refresh` is now read-only: `--shape-report` diffs key paths/types and runs a leak audit; fixture updates happen only inside iterations that change client types, fixtures, and tests together. Procedure lives in the `/api-drift-sweep` skill (iter-78) |
| 2026-07-10 | `release.yml` replaced with a thin caller of `ractive/release-workflows` | hoppy, hyalo, and ff-rdp carried near-duplicate `release.yml` files that needed manual, lockstep patching for every pipeline fix. The reusable pipeline in `ractive/release-workflows@v0.1.0` centralises build/package/publish logic behind a `workflow_call` interface; hoppy's real completions/man generation, deb/rpm asset layout, 5-target matrix (no musl — OpenSSL linkage), and Homebrew bore-cli caveats are preserved via the caller's `pre-package-command`/`targets`/`homebrew-caveats` inputs. Also added `Cross.toml`'s `[build.env] passthrough` for `GIT_COMMIT`/`GIT_COMMIT_DATE` (previously missing — `build.rs` needs them for hermetic provenance on the cross-compiled aarch64 target) and a `publish-crates.yml` recovery workflow that hoppy didn't have before (iter-79) |
