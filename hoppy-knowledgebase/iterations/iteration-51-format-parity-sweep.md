...
- [x] Grep every subcommand handler for `--format` handling. Build a
      matrix: command × {table, text, json} × honoured?
- [x] Note the convention each command currently uses for casing.
- [x] Paste the matrix into the PR body.

### 2. Fix `db` v2-style commands

- [x] All `db` subcommands honour `--format json` and emit
      PascalCase keys matching other domains.
- [x] `--format text` uses the same key names as `--format json`.

### 3. Fix mutation subcommands

- [x] `create`/`update`/`delete` across all domains honour
      `--format json` — print the API response (or a success
      envelope) as JSON, not the prose confirmation.
- [x] Default (no `--format`) keeps current prose behaviour.

### 4. Fix `container region` field-name divergence

- [x] `--format text` uses PascalCase (`HasAnycastSupport`,
      `HasCapacity`) to match `--format json` after the casing fix.
- [x] `--format table` headers map cleanly (document the mapping in
      help text if abbreviated).

### 5. Tests

- [x] E2E snapshot per fixed command × format.
- [x] Add a coverage test that asserts every subcommand returns
...
- [x] Every subcommand emits valid JSON when `--format json` is
      passed.
- [x] Field names match between `--format text` and `--format json`.
- [x] Snapshot tests cover the previously-broken commands.
