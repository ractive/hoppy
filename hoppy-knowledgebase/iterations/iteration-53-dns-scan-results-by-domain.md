...
- [x] E2E snapshot for `scan start --domain` then `scan results
      --job-id` flow.
- [x] Error-case snapshot when no arg supplied.
- [x] Live test (feature `live-api`) end-to-end.
...
- [x] `scan start --domain <d>` followed by `scan results --domain <d>`
