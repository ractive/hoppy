...
- [ ] Live E2E: create SZ → create PZ bound to SZ → assert `OriginUrl == ""` and `StorageZoneId` matches → cleanup *(deferred — covered by wiremock + ArgGroup unit tests; live test gated behind `live-api` feature)*
...
- [ ] Live E2E: `storage-zone get --id <id>` returns a Password that round-trips successfully against the storage API endpoint *(deferred — covered by wiremock; live test gated behind `live-api` feature)*
...
- [ ] Investigate: is the bunny API filtering them out, or is it a flag we're not passing? Capture and document in `api/bunny-api-quirks.md`. *(deferred — needs live API exploration; documented in quirks as a limitation to look at next time)*
- [ ] If a flag exists, add `pull-zone list --include-managed` (default off) to show them. *(deferred)*
- [ ] If no flag, document the limitation in `pull-zone list --help` long_help and cross-reference `container endpoint list`. *(deferred)*
...
- [ ] Audit all subcommands for the identifier flag they take. Produce a small table in the iteration as the canonical reference. *(deferred — non-trivial cross-cutting refactor, separate iteration)*
- [ ] Pick one convention (proposal: `--id` for the resource the subcommand acts on; `--<resource>-id` for cross-resource references). Document in `decision-log.md`. *(deferred)*
- [ ] Apply renames with `#[arg(long, alias = "<old>")]` so existing scripts keep working — no breaking changes in this iteration. *(deferred)*
...
- [ ] Document feature gaps inline (e.g. if a flag is missing because the API supports it but hoppy doesn't yet, say so in help text rather than failing with a 400). *(deferred — no specific gap surfaced this iteration)*
...
- [ ] Deserialization errors translate `column N` to field name where possible *(deferred — superseded by lossy enum deserializer making the panic path inaccessible for the common case)*
- [ ] When an unknown enum value is the cause, hint at a hoppy upgrade *(deferred — same)*
